...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def union(that: TweetSet): TweetSet = filterAcc(p => true,that)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = all.filter(t => google.exists(t.text.contains) )
  lazy val appleTweets: TweetSet = all.filter(t => apple.exists(t.text.contains) )
